...
The full 2D companion to the cash-arb export, rebuilt and uploaded at the end of every gated run by `build_gfi_master()`. It uses the same xlsx/Josh date-precedence rule, pivot key, `MTD` exclusion, local folder, and S3 prefix, but keeps **every instrument whose name begins with `TD`** and makes each route a value column. The four identifier columns remain `periodType, date, period, uom`; route columns are dynamically derived and naturally sorted. The current source vocab contains both `TD3` (xlsx history) and `TD3C` (Josh), so they remain separate columns rather than silently imposing an alias.
