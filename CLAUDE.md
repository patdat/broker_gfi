...
2. **Parse** (`utils/read_csv_file.py`, `utils/read_xlsx_file.py`): reshape each raw file into long format (`melt`), keep only routes containing `TD` (dirty tanker routes; filtered with `na=False` so a blank/trailing row can't crash it), normalize the XLSX feed's raw `TD3` label to the canonical `TD3C`, and resolve period codes via the lookup (below).
...
- **Parse** (`utils/read_josh_file.py`): the Curves file is transposed vs. the Braemar xlsx (instruments across columns, tenors down rows) and stacks two curve blocks — **World Scale** (`uom=WSC`) and **USD/Tonne** (`uom=PMT`) — plus a `WSFR` flat-rate row and a `Linked` **BITR** table. Keeps a curated instrument set via **`lookup/josh_instruments.csv`** (`header,instrument,lsm`). Directly-normalized routes: `TD3 C`→`TD3C`, `TD20 inc`→`TD20`, plus `TD8`, `TD19`, `TD22`, `TD28`, `TD34`. Three routes are taken from a **differently-based column** because the broker's own same-named column uses a wrong flat-rate basis (and prints junk near month-end):
...
**`TD34`** is a late addition to the Curves file — it first appears on **2026-08-24**, is absent 8/25–9/11, and is present again from **2026-09-14**. Its column header is a bare `TD34` (no normalization needed) and it is `uom=WSC`/`PMT` like the other WS routes. It has **no `Linked`-table row, so no BITR** (same as `TD25E`). The broker leaves the far forward month unquoted as a literal **`0`** (e.g. `2027-04-01` on 9/14–9/16); zeros are carried through as-is, matching the existing behaviour for other routes.
...
### All-route parquet (`utils/gfi_master.py` → `gfi_master.parquet`)
The full 2D companion to the cash-arb export, rebuilt and uploaded at the end of every gated run by `build_gfi_master()`. It uses the same xlsx/Josh date-precedence rule, pivot key, `MTD` exclusion, local folder, and S3 prefix, but keeps instruments whose names begin with `TD` (except explicitly excluded `TD8` and `TD25E`) and makes each retained route a value column. The four identifier columns remain `periodType, date, period, uom`; route columns are dynamically derived and naturally sorted. `TD3C` is canonical in both feeds: the XLSX feed's raw `TD3` label is normalized during parsing, and the parquet builder also applies the alias defensively for older masters.
